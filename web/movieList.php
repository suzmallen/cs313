...
function get_db() {
	...
	try {
		...
		if (!isset($dbUrl) || empty($dbUrl)) {
			...
			$dbUrl = "postgres://php_movie_guy:orange@localhost:5432/movies";
...
$mydb = get_db();
$statement = $mydb->prepare("SELECT id, title FROM movie");
$statement->execute();
       ...
       <?php while ($row= $statement->fetch(PDO::FETCH_ASSOC))
{
    ...
    echo "<p><a href=moviedetails.php?id=$id>$title</a></p>";
}
    ?>
    </body>
</html>